...
public class EShopController {
    ...
    public static void addItemToStorage(Item item, int count) throws IllegalArgumentException{
        if (count < 1){
            throw new IllegalArgumentException("Item count should be more than 0");
        }
        storage.insertItems(item, count);
    }

    public static ArrayList<ItemStock> getItemFromStorage() {
        ArrayList<ItemStock> result = new ArrayList(storage.getStockEntries());
       return result;
    }

    public static ArrayList<ShoppingCart> getCart() {
        return carts;
    }

    public static PurchasesArchive getPurchasesArchive() {
        return archive;
    }
